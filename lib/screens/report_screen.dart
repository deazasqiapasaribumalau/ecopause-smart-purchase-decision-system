...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _wishlist = res[1] as List<WishlistItem>;
        _logs = res[2] as List<ShoppingLog>;
        _loading = false;
      });
    }
  }
          ...
          : RefreshIndicator(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    // Header
                    _buildHeader(months, now),
                    const SizedBox(height: 16),

                    // KPI Grid - DIBERI TINGGI MAKSIMUM
                    ConstrainedBox(
                    ...
                    // Category Chart
                    if (_catCount.isNotEmpty) ...[
                      _buildCategoryChart(),
                      const SizedBox(height: 16),
                    ],

                    // Monthly Shopping Summary
                    _buildShoppingSummary(),
  ...
  // ========== HEADER ==========
  Widget _buildHeader(List<String> months, DateTime now) {
    return Row(
      children: [
        Expanded(
          child: Text(
    ...
    return GridView.count(
      ...
      children: [
        ...
        _KPICard(
          '☁️',
          'Estimasi CO₂',
          '${_totalCO2.toStringAsFixed(1)} kg',
          AppTheme.sand,
        ),
      ],
    );
    ...
    return EcoCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Berapa banyak pembelian yang berhasil ditahan?',
            style: GoogleFonts.nunito(
              fontSize: 12,
              color: AppTheme.grey,
            ),
          ...
          if (_wishlist.isEmpty)
            ...
            if (_wishlistSkips > 0) ...[
              ...
              Container(
                ...
                child: Text(
                  '🌟 Kamu berhasil menahan $_wishlistSkips pembelian impulsif lewat cooling period!',
                  style: GoogleFonts.nunito(
                    fontSize: 12,
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Text(
              '$count item (${(pct * 100).toStringAsFixed(0)}%)',
              style: GoogleFonts.nunito(
                fontSize: 11,
                fontWeight: FontWeight.w700,
                color: color,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: top.asMap().entries.map((e) {
                      final lbl = e.value.key.length > 16
                          ? '${e.value.key.substring(0, 14)}...'
                          : e.value.key;
                      return Padding(
  ...
  Widget _buildShoppingSummary() {
    final totalSpent = _logs.fold(0.0, (s, l) => s + l.price);
    final onlineCount = _logs.where((l) => l.isOnline).length;
    final offlineCount = _logs.where((l) => !l.isOnline).length;
    ...
    return EcoCard(
      child: Column(
        ...
        children: [
          ...
          _summaryRow('📦', 'Total pembelian dicatat', '${_logs.length} item'),
          const SizedBox(height: 8),
          _summaryRow('💸', 'Total pengeluaran', 'Rp ${_fmtP(totalSpent)}'),
          const SizedBox(height: 8),
          _summaryRow('♻️', 'Sampah kemasan dihasilkan', '${_totalWaste.toStringAsFixed(2)} kg'),
          const SizedBox(height: 8),
          _summaryRow(
            '🚚',
            'Belanja online vs offline',
            '$onlineCount vs $offlineCount',
          ),
        ],
      ),
    );
