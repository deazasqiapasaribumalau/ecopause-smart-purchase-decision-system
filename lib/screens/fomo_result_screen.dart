...
class FomoResultScreen extends StatelessWidget {
  ...
  String get _verdict => eval.decision == 'buy'
      ? '✅ Layak Dibeli'
      : eval.decision == 'skip'
          ? '🚫 Lebih Baik Dilewati'
          : '⏳ Masuk Wishlist Dulu';
  String get _verdictDesc => eval.decision == 'buy'
      ? 'Kebutuhan kamu terhadap barang ini cukup kuat dan rasional. Tetap pertimbangkan dampak lingkungan ya!'
      : eval.decision == 'skip'
          ? 'Sepertinya pembelian ini didorong lebih banyak oleh FOMO daripada kebutuhan nyata. Coba tunda dan lihat besok apakah masih mau beli.'
          : 'Skor kamu berada di zona abu-abu. Masukkan ke wishlist dengan cooling period untuk memastikan kamu benar-benar butuh.';
  Color get _verdictColor => eval.decision == 'buy'
      ? AppTheme.forestMid
      : eval.decision == 'skip'
          ? AppTheme.terra
          : AppTheme.sand;
          ...
          children: [
              ...
              child: Column(
                children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              spacing: 8,
                              children: [
                                EcoChip(label: eval.category),
                                EcoChip(
                                  label: 'Rp ${_fmtP(eval.price)}',
                  ...
                  Row(
                    ...
                    children: [
                      ScoreRing(
                        score: eval.needScore,
                        label: 'Need Score',
                        color: AppTheme.needColor(eval.needScore),
                      ),
                      Container(
                        width: 1,
                        height: 80,
                      ...
                      ScoreRing(
                        score: eval.fomoScore,
                        label: 'FOMO Score',
                        color: AppTheme.fomoColor(eval.fomoScore),
                      ),
                    ],
                  ),
                ],
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: _verdictColor.withOpacity(0.4),
                  width: 2,
                ),
              ),
              child: Column(
            ...
            // ========== SECTION 4: Buy or Borrow ==========
            if (_borrowAdvice != null) ...[
              EcoCard(
                borderColor: AppTheme.sky.withOpacity(0.6),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
  ...
  Widget _buildImageSection() {
    return EcoCard(
      borderColor: AppTheme.sage.withOpacity(0.4),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
  ...
  Widget _buildImageWidget() {
    final imagePath = eval.imagePath!;

    // Cek apakah path dimulai dengan 'http' (gambar dari internet)
  ...
  void _addToWishlist(BuildContext context) async {
    int? cooling = await showDialog<int>(
      context: context,
      builder: (_) => const _CoolingDialog(),
    );
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '${eval.itemName} ditambahkan ke wishlist ($cooling hari cooling)',
          ),
          backgroundColor: AppTheme.forestMid,
        ),
      );
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => WishlistScreen(auth: auth)),
        (r) => r.isFirst,
      );
    }
  }
...
// ========== Dialog Cooling Period ==========
class _CoolingDialog extends StatefulWidget {
  const _CoolingDialog();

  @override
  State<_CoolingDialog> createState() => _CoolingDialogState();
}
...
class _CoolingDialogState extends State<_CoolingDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Tunggu beberapa hari sebelum memutuskan — ini membantu mengurangi pembelian impulsif.',
              style: GoogleFonts.nunito(
                fontSize: 13,
                color: AppTheme.ink,
              ),
